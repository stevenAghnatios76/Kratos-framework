...
1. PARSE arguments from $ARGUMENTS:
   - Extract story_key (optional): pattern like E1-S1, E2-S3, etc. (alphanumeric with hyphens, matching E\d+-S\d+)
   - Check for "yolo" keyword (case-insensitive): if present, set yolo_mode=true
   - If yolo_mode=true but no story_key found: ASK the user for the story key — YOLO mode requires a story key
2. LOAD the FULL {project-root}/_kratos/core/engine/workflow.xml
3. READ its entire contents — this is the CORE OS
4. Pass {project-root}/_kratos/lifecycle/workflows/4-implementation/create-story/workflow.yaml as 'workflow-config'
